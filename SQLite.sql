Table Inicio {
  ...
  Save_Meio_Ambiente varchar
  Parceiros text [note: 'Greenpeace
...
Table Contato {
  id integer [primary key]
  Email varchar [note: 'savemeioambiente@gmail.com']
  Telefone Integer [note: '4002-8922']
  Redes_Sociais text [note: 'Facebook: Save Meio Ambiente
...
Table Sobre {
  ...
  Sobre_Nos text [note: 'Somos um grupo de apaixonados pelo meio ambiente que acredita firmemente na importância da conservação e proteção do nosso planeta. Nosso objetivo é promover a conscientização sobre os desafios ambientais que enfrentamos e inspirar ações positivas para um futuro mais sustentável.']
 
  Nossa_Missao text [note: 'Educar: Acreditamos que a educação é a chave para a mudança. Estamos comprometidos em fornecer informações precisas e relevantes sobre questões ambientais para que as pessoas possam tomar decisões informadas em suas vidas diárias.
...
Ação: Não basta apenas falar sobre o meio ambiente; é preciso agir. Promovemos ações práticas que todos podem adotar para reduzir seu impacto ambiental, como práticas de conservação, redução do uso de plástico e promoção da energia limpa.']
  ...
  Nossos_Valores text [note: 'Sustentabilidade: Estamos comprometidos em viver de maneira sustentável e incentivamos outros a fazerem o mesmo. Acreditamos que a sustentabilidade é essencial para garantir um planeta saudável para as gerações futuras.
...
Table Noticias {
  id integer [primary key]
  Meio_Ambiente text [note: "Operação Mata Atlântica aplica R$ 9 milhões em multas em MG
...
Ref: "Inicio"."id" < "Contato"."id" 
Ref: "Inicio"."id" < "Sobre"."id"
Ref: "Inicio"."id" < "Noticias"."id"
